...
export function UpdateAddress({ cancelHandleUpdate, currentAddress }: { cancelHandleUpdate: () => void; currentAddress: ShowAddressContract | null }) {
    ...
    const formik = useFormik<ShowAddressContract>({
        ...
        onSubmit: async (formData, { resetForm }) => {
            ...
            try {
                const id = currentAddress?._id;
                const { street, city, state, country, postalCode, mobile, name } = formData
                const response = await axios.put(`http://localhost:8000/user/update-address/${id}`,
    ...
    return (
        <>
            <form onSubmit={formik.handleSubmit} className='address-form'>
                <h6 className='text-center fs-4'>Update Address</h6>
                <hr />
                ...
                <div className='form-group mb-4 button-containerr mt-3'>
                    <Button variant="contained" type='submit' className='form-control ' disabled={isSubmitting} >Submit</Button>
                    <Button variant="contained" className='form-control bg-danger w-40' onClick={cancelHandleUpdate} >Cancel</Button>
                </div>
            </form>
        </>
    )
